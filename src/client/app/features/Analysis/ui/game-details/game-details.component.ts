...
export class GameDetailsComponent extends SubscriptionHolder implements OnInit, OnDestroy {
  protected readonly playersMap: Map<string, RosterPlayer> = new Map((this.activatedRoute.snapshot.data['players'] as RosterPlayer[]).map((player: RosterPlayer) => [player.playerID, player]));
